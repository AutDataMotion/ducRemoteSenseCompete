            ...
            try:
                ...
                try:
                    ...
                    try:
                       ...
                       #TODO: 目前无法发送，稍后再试
                       send_mail("恭喜你成功报名参加本届比赛", "{}, 恭喜您成功报名参加本次大赛，您注册的手机号码为{}, 作为队长，您最多可邀请4名队员组队参加，队伍邀请码为{}，请积极准备比赛，预祝取得好成绩".format(serializer.data['name'], serializer.data['phone_number'], team.invite_code), "rssrai2019@hotmail.com", [serializer.data["email"]], fail_silently=False)
                    except Exception as e:
                        #发送邮件失败注册信息全部删除
